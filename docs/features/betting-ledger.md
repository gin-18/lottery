...
胆拖号码组的标题操作区在“删除号码组”左侧提供一个批量按钮：当拖码未覆盖全部可选拖码时显示“全选拖码”，点击后选择 1-80 中所有未被选为胆码的号码；当拖码已覆盖全部可选拖码时显示“清空拖码”，点击后清空拖码。胆码和拖码始终互斥；用户新增胆码后，该号码应从拖码中排除，拖码按钮状态按排除胆码后的当前拖码集合重新判断。

胆拖号码组的胆码选择按当前玩法限制数量，上限为玩法要求数量减一。当前表单编辑会话中，胆码达到上限后继续点选新的胆码，会替换最后一次点选的胆码；点选顺序只作为表单临时交互状态，不保存进投注记录。切换玩法导致胆码上限变小时，按点选顺序保留较早选择的胆码并移除超出上限的较新胆码；被移除的胆码不自动加入拖码。重新编辑已保存记录时，以记录中的胆码数组顺序作为本次表单会话的初始点选顺序。
